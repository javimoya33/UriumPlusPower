<?php
namespace TrxAddons\AiHelper\Media;
...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		...
		function __construct() {
			...
			add_filter( 'media_upload_tabs', array( $this, 'add_tab_image_generator' ) );
			add_action( 'media_upload_trx_ai_helper_image_generator', array( $this, 'show_tab_image_generator' ) );
...
//				$vars['msg_ai_helper_error'] = esc_html__( "AI Helper error", 'trx_addons' );
		...
		/**
		 * Add a new tab 'Image Generator' to the media selector
		 */
		function add_tab_image_generator( $tabs ) {
			if ( self::is_allowed() ) {
				$tabs['trx_ai_helper_image_generator'] = esc_html__( 'Image Generator', 'trx_addons' );
			}
			return $tabs;
		}

		/**
		 * Show tab 'Image Generator'
		 */
		function show_tab_image_generator() {
			echo '<div id="media-upload-header">Test</div>';
		}
