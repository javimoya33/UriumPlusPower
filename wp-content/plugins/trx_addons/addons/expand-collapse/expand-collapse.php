...
if ( ! function_exists( 'trx_addons_expand_collapse_load_scripts_front' ) ) {
	...
	function trx_addons_expand_collapse_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && ! $preview_gb && ( ( ! $optimize && $debug ) || ( $optimize && $need ) ) ) {
			$loaded = true;
			wp_enqueue_style(  'trx_addons-expand-collapse', trx_addons_get_file_url( TRX_ADDONS_PLUGIN_ADDONS . 'expand-collapse/expand-collapse.css' ), array(), null );
			wp_enqueue_script( 'trx_addons-expand-collapse', trx_addons_get_file_url( TRX_ADDONS_PLUGIN_ADDONS . 'expand-collapse/expand-collapse.js' ), array('jquery'), null, true );
			do_action( 'trx_addons_action_load_scripts_front', $force, 'expand_collapse' );
		}
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'expand_collapse', 2 );
		}
	}
}
...
//	add_action( 'trx_addons_action_show_layout_from_cache', 'trx_addons_audio_effects_check_in_html_output', 10, 1 );
	...
	function trx_addons_expand_collapse_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'expand_collapse' )
