...
if ( ! function_exists( 'trx_addons_edd_load_scripts_front' ) ) {
	...
	function trx_addons_edd_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && ! $preview_gb && ( ( ! $optimize && $debug ) || ( $optimize && $need ) ) ) {
			$loaded = true;
			wp_enqueue_style(  'trx_addons-edd', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_API . 'easy-digital-downloads/easy-digital-downloads.css'), array(), null );
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'edd' );
		}
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'edd', 2 );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_edd_load_scripts_front_responsive' ) ) {
	...
	function trx_addons_edd_load_scripts_front_responsive( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && trx_addons_need_frontend_scripts( 'edd' )
			||
			...
			if ( ! trx_addons_is_preview( 'gutenberg' ) ) {
				wp_enqueue_style(  'trx_addons-edd-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_API . 'easy-digital-downloads/easy-digital-downloads.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'edd', 'md' ) );
			}
		}
	}
}
...
//	add_action( 'trx_addons_action_show_layout_from_cache', 'trx_addons_edd_check_in_html_output', 10, 1 );
	...
	function trx_addons_edd_check_in_html_output( $content = '' ) {
		if ( trx_addons_exists_edd()
			&& ! trx_addons_need_frontend_scripts( 'edd' )
...
if ( ! function_exists( 'trx_addons_edd_filter_head_output' ) ) {
	...
	function trx_addons_edd_filter_head_output( $content = '' ) {
		if ( trx_addons_exists_edd()
			...
			&& ! trx_addons_need_frontend_scripts( 'edd' )
			&& apply_filters( 'trx_addons_filter_remove_3rd_party_styles', true, 'edd' )
		) {
			$content = preg_replace( '#<link[^>]*href=[\'"][^\'"]*/easy-digital-downloads/[^>]*>#', '', $content );
		}
		return $content;
	}
}
...
if ( ! function_exists( 'trx_addons_edd_filter_body_output' ) ) {
	...
	function trx_addons_edd_filter_body_output( $content = '' ) {
		if ( trx_addons_exists_edd()
			...
			$content = preg_replace( '#<link[^>]*href=[\'"][^\'"]*/easy-digital-downloads/[^>]*>#', '', $content );
			$content = preg_replace( '#<script[^>]*src=[\'"][^\'"]*/easy-digital-downloads/[^>]*>[\\s\\S]*</script>#U', '', $content );
		}
		return $content;
	}
}
