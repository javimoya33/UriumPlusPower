...
if ( !function_exists( 'trx_addons_cpt_dishes_load_scripts_front' ) ) {
	...
	function trx_addons_cpt_dishes_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && ! $preview_gb && ( ( ! $optimize && $debug ) || ( $optimize && $need ) ) ) {
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'cpt_dishes' );
		}
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'cpt_dishes', 2 );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_cpt_dishes_load_scripts_front_responsive' ) ) {
	...
	function trx_addons_cpt_dishes_load_scripts_front_responsive( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && trx_addons_need_frontend_scripts( 'cpt_dishes' )
			||
			current_action() != 'wp_enqueue_scripts' && $force === true
			)
		) {
			$loaded = true;
			wp_enqueue_style( 'trx_addons-cpt_dishes-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'dishes/dishes.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'cpt-dishes', 'md' ) );
		}
	}
}
...
if ( !function_exists( 'trx_addons_cpt_dishes_check_in_html_output' ) ) {
	...
	function trx_addons_cpt_dishes_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'cpt_dishes' )
