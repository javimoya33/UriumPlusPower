	...
	function trx_addons_cpt_layouts_create( $check = true ) {
		...
		if ( count( $layouts ) > 0 ) {
			...
			foreach( $layouts as $slug => $layout ) {
				...
				$exists = false;
				if ( $check ) {
					$post = get_page_by_title( $layout['name'], OBJECT, TRX_ADDONS_CPT_LAYOUTS_PT );
...
if ( !function_exists( 'trx_addons_cpt_layouts_load_responsive_styles' ) ) {
	...
	function trx_addons_cpt_layouts_load_responsive_styles() {
		if (trx_addons_exists_page_builder() && trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-cpt_layouts-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'layouts/layouts.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'cpt-layouts', 'xl' ) );
		}
	}
}
