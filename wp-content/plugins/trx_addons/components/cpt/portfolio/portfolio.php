...
if ( ! function_exists( 'trx_addons_cpt_portfolio_load_scripts_front2' ) ) {
	add_action( 'wp_enqueue_scripts', 'trx_addons_cpt_portfolio_load_scripts_front2', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY - 1 );
	function trx_addons_cpt_portfolio_load_scripts_front2() {
...
if ( !function_exists( 'trx_addons_cpt_portfolio_load_scripts_front' ) ) {
	...
	function trx_addons_cpt_portfolio_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && ! $preview_gb && ( ( ! $optimize && $debug ) || ( $optimize && $need ) ) ) {
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'cpt_portfolio' );
		}
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'cpt_portfolio', 2 );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_cpt_portfolio_load_scripts_front_responsive' ) ) {
	...
	function trx_addons_cpt_portfolio_load_scripts_front_responsive( $force = false ) {
		...
		if ( ! $loaded && (
			...
			$loaded = true;
			wp_enqueue_style( 'trx_addons-cpt_portfolio-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'portfolio/portfolio.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'cpt-portfolio', 'lg' ) );
		}
	}
}
...
if ( !function_exists( 'trx_addons_cpt_portfolio_check_in_html_output' ) ) {
	...
	function trx_addons_cpt_portfolio_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'cpt_portfolio' )
