...
class trx_addons_widget_categories_list extends TRX_Addons_Widget {
	...
	function widget($args, $instance) {
		...
		if ( empty( $categories ) || is_wp_error( $categories ) || count( $categories ) == 0 ) return;
	...
	function update($new_instance, $instance) {
		...
		$instance['cat_list'] = ! empty( $new_instance['cat_list'] )
									...
									: '';
		$instance['show_thumbs'] = !empty($new_instance['show_thumbs']) && (int)$new_instance['show_thumbs'] > 0 ? 1 : 0;
		$instance['show_posts'] = !empty($new_instance['show_posts']) && (int)$new_instance['show_posts'] > 0 ? 1 : 0;
		$instance['show_children'] = !empty($new_instance['show_children']) && (int)$new_instance['show_children'] > 0 ? 1 : 0;
...
if ( !function_exists( 'trx_addons_widget_categories_list_load_scripts_front' ) ) {
	...
	function trx_addons_widget_categories_list_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'widget_categories_list', 2 );
		}
	}
}
