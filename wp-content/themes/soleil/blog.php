...
if ( function_exists( 'soleil_elementor_is_preview' ) && soleil_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			soleil_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	soleil_new_main_query(
		array(
			'post_type'      => soleil_get_theme_option( 'post_type' ),
			'category'       => soleil_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'soleil_filter_get_template_part', soleil_blog_archive_get_template() ) );
}
