<div class="front_page_section front_page_section_testimonials<?php
	$soleil_scheme = soleil_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $soleil_scheme ) && ! soleil_is_inherit( $soleil_scheme ) ) {
		echo ' scheme_' . esc_attr( $soleil_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( soleil_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( soleil_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$soleil_css      = '';
		$soleil_bg_image = soleil_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$soleil_anchor_icon = soleil_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$soleil_anchor_text = soleil_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( soleil_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$soleil_css      = '';
			$soleil_bg_mask  = soleil_get_theme_option( 'front_page_testimonials_bg_mask' );
			$soleil_bg_color_type = soleil_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $soleil_bg_color_type ) {
				$soleil_bg_color = soleil_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$soleil_caption = soleil_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $soleil_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $soleil_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $soleil_caption, 'soleil_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$soleil_description = soleil_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $soleil_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $soleil_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $soleil_description ), 'soleil_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! soleil_exists_trx_addons() ) {
						...
						soleil_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
