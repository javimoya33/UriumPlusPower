...
if ( ! function_exists( 'soleil_storage_get' ) ) {
	/**
	 ...
	 */
	function soleil_storage_get( $var_name, $default = '' ) {
		global $SOLEIL_STORAGE;
		return isset( $SOLEIL_STORAGE[ $var_name ] ) ? $SOLEIL_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'soleil_storage_set' ) ) {
	/**
	 ...
	 */
	function soleil_storage_set( $var_name, $value ) {
		global $SOLEIL_STORAGE;
		$SOLEIL_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'soleil_storage_empty' ) ) {
	/**
	 ...
	 */
	function soleil_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $SOLEIL_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $SOLEIL_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $SOLEIL_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $SOLEIL_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'soleil_storage_isset' ) ) {
	/**
	 ...
	 */
	function soleil_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $SOLEIL_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $SOLEIL_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $SOLEIL_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $SOLEIL_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'soleil_storage_unset' ) ) {
	/**
	 ...
	 */
	function soleil_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $SOLEIL_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $SOLEIL_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $SOLEIL_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $SOLEIL_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'soleil_storage_inc' ) ) {
	/**
	 ...
	 */
	function soleil_storage_inc( $var_name, $value = 1 ) {
		global $SOLEIL_STORAGE;
		if ( empty( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = 0;
		}
		$SOLEIL_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'soleil_storage_concat' ) ) {
	/**
	 ...
	 */
	function soleil_storage_concat( $var_name, $value ) {
		global $SOLEIL_STORAGE;
		if ( empty( $SOLEIL_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'soleil_storage_get_array' ) ) {
	/**
	 ...
	 */
	function soleil_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $SOLEIL_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $SOLEIL_STORAGE[ $var_name ][ $key ] ) ? $SOLEIL_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $SOLEIL_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $SOLEIL_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'soleil_storage_set_array' ) ) {
	/**
	 ...
	 */
	function soleil_storage_set_array( $var_name, $key, $value ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$SOLEIL_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'soleil_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function soleil_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ][ $key ] ) ) {
			$SOLEIL_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$SOLEIL_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$SOLEIL_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'soleil_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function soleil_storage_merge_array( $var_name, $key, $value ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$SOLEIL_STORAGE[ $var_name ] = array_merge( $SOLEIL_STORAGE[ $var_name ], $value );
		} else {
			$SOLEIL_STORAGE[ $var_name ][ $key ] = array_merge( $SOLEIL_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'soleil_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function soleil_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			soleil_array_insert_after( $SOLEIL_STORAGE[ $var_name ], $after, $key );
		} else {
			soleil_array_insert_after( $SOLEIL_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'soleil_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function soleil_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			soleil_array_insert_before( $SOLEIL_STORAGE[ $var_name ], $before, $key );
		} else {
			soleil_array_insert_before( $SOLEIL_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'soleil_storage_push_array' ) ) {
	/**
	 ...
	 */
	function soleil_storage_push_array( $var_name, $key, $value ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $SOLEIL_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $SOLEIL_STORAGE[ $var_name ][ $key ] ) ) {
				$SOLEIL_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $SOLEIL_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'soleil_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function soleil_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $SOLEIL_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $SOLEIL_STORAGE[ $var_name ] ) && is_array( $SOLEIL_STORAGE[ $var_name ] ) && count( $SOLEIL_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $SOLEIL_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $SOLEIL_STORAGE[ $var_name ][ $key ] ) && is_array( $SOLEIL_STORAGE[ $var_name ][ $key ] ) && count( $SOLEIL_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $SOLEIL_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'soleil_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function soleil_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( empty( $SOLEIL_STORAGE[ $var_name ][ $key ] ) ) {
			$SOLEIL_STORAGE[ $var_name ][ $key ] = 0;
		}
		$SOLEIL_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'soleil_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function soleil_storage_concat_array( $var_name, $key, $value ) {
		global $SOLEIL_STORAGE;
		if ( ! isset( $SOLEIL_STORAGE[ $var_name ] ) ) {
			$SOLEIL_STORAGE[ $var_name ] = array();
		}
		if ( empty( $SOLEIL_STORAGE[ $var_name ][ $key ] ) ) {
			$SOLEIL_STORAGE[ $var_name ][ $key ] = '';
		}
		$SOLEIL_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'soleil_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function soleil_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $SOLEIL_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $SOLEIL_STORAGE[ $var_name ] ) ? $SOLEIL_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $SOLEIL_STORAGE[ $var_name ] ) ? $SOLEIL_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'soleil_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function soleil_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $SOLEIL_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $SOLEIL_STORAGE[ $var_name ]->$prop ) ? $SOLEIL_STORAGE[ $var_name ]->$prop : $default;
	}
}
