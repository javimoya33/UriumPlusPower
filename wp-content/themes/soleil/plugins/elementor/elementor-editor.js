...
jQuery( document ).ready(
	function() {
		...
		soleil_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="soleil_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.soleil_post_editor' );
						if ( link.length > 0 ) {
							soleil_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.soleil_post_editor', function(e) {
						soleil_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('soleil_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function soleil_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
