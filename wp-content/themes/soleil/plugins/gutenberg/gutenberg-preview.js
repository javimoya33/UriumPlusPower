...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			soleil_create_observer( 'soleil-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				soleil_gutenberg_editor_init();
			} );
		} else {
		 	soleil_gutenberg_editor_init();
		}
	...
	function soleil_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'soleil_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'soleil_inited' ) ) {
	...
	// Init on page load
	function soleil_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = soleil_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + SOLEIL_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + SOLEIL_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in SOLEIL_STORAGE['override_classes'] ) {
				$obj.addClass( SOLEIL_STORAGE['override_classes'][i].replace( '%s', SOLEIL_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( soleil_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && SOLEIL_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'soleil_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			soleil_remove_observer( 'soleil-check-editor-styles-wrapper' );
			soleil_create_observer( 'soleil-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'soleil_inited' );
	}
} );
